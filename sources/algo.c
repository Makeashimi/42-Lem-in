...
void	close_here(t_ant *ant, t_room *room)
{
	...
	while (tmp_room != NULL)
	{
		if (check_close(tmp_room->path) == 1 ||
			ft_strcmp(tmp_room->name, ant->here) == 0)
		{
...
void	smaller_distance(t_ant *ant, t_room *room)
{
	...
	while (tmp != NULL)
	{
		...
		while (cpy != NULL)
		{
			if (ft_strcmp(tmp->name, ant->end) != 0 && cpy->close == 0 &&
				(cpy->distance >= 0 && cpy->distance < distance))
			{
